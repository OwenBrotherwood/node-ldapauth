 ...
 *        search.
 *    requireGroupDn {String} Optional. The DN of LDAP group, to which user must
 *        belong for succesful authentication. If not specified, group membership
 *        will not be required. E.g. 'cn=admins,ou=groups,o=example.com'
...
LdapAuth.prototype.authenticate = function (username, password, callback) {
    ...
    self._findUser(username, function (err, user) {
        ...
        self._userClient.bind(user.dn, password, function (err) {
            ...
            if (self.opts.requireGroupDn) {
                var searchFilter = '(|(objectClass=groupOfUniqueNames)(objectClass=groupOfNames))'
                var opts = {filter: searchFilter, scope: 'base', attributes: ['uniqueMember','member']};
                self._userClient.search(self.opts.requireGroupDn, opts,
                                         function (err, result) {
                    if (err) {
                        self.log && self.log.trace('ldap authenticate: search error: %s', err);
                        return callback(err);
